...
**环境要求：** Linux VPS · Python ≥ 3.12 · [uv](https://docs.astral.sh/uv/)
...
> 💡 通过 [LiteLLM](https://github.com/BerriAI/litellm) 支持主流模型——通义千问、Gemini、Claude、DeepSeek、GPT 等。在 `.env` 里改一项配置就能切换。

> **最快上手（不需要 Syncthing 或 Composio）：** 在 `.env` 里填一个 API Key，然后跑 `uv run agent`。两分钟内就能在终端里得到一个可用的 Agent。准备好了再接 Syncthing（文件同步）和 Composio（应用触发）。
...
| 组件 | 文档 |
|:---|:---|
| 桌面端 | [docs/desktop_setup.md](docs/desktop_setup.md) |
...
| Composio 应用集成 | [docs/composio_setup.md](docs/composio_setup.md) |
